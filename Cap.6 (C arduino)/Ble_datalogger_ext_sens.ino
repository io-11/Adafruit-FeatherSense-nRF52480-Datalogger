/* nrf52840 - Bluetooth Data Logger inspired by MJRoBot (Marcelo Rovai)
 * https://www.hackster.io/mjrobot/sensor-datalogger-50e44d
...
int minPeriod=350;  /*definisco il periodo minimo in millisecondi di lettura dei dati. Dipende dal sensore che si è collegato. 
            Nel mio caso LM75A ha un periodo minimo di 300ms. Lo metto all'inizio in modo da permettere facili modifiche */
...
void loop() 
{
  //questo if mostra 1 sola volta il messaggio di riconnessione sul serial monitor quando la connessione è persa
  ...
  {    
    ...
    if(millis()>=preMillis+minPeriod) //se il tempo letto ora è maggiore a quello precedente+il periodo indicato all'inizio,
                                  //entro nel corpo dell'if
    {
  ...
  } //resta nel while quando è connesso
} 
...
{
  Wire.beginTransmission(LM75A_ADDRESS); //prepara la comunicazione con il sensore
